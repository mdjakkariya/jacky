...
def test_build_llm_wires_lexical_selector_by_default() -> None:
    ...
    harness = _build_llm(Settings(context_tokens=4096), ToolRegistry(), NullTranscript(), None)
...
def test_build_llm_anthropic_off_falls_back_to_local_without_key(
    ...
    assert isinstance(harness, AgentHarness)
    assert isinstance(harness._model, OllamaLanguageModel)
