...
```bash
git checkout -b release/v0.7.0 main
make release-cli VERSION=0.7.0
make changelog-cli VERSION=0.7.0
git commit -am "chore(release): v0.7.0" && gh pr create --fill   # merge when green
git checkout main && git pull
git tag v0.7.0 && git push origin v0.7.0     # CI builds wheel + 5 jack binaries
```
...
What ends up attached to the `v0.7.0` Release:

- `autobot-*.whl` / `autobot-*.tar.gz` — the Python engine (wheel + sdist).
- `jack-0.7.0-<os>-<arch>.(tar.gz|zip)` + a matching `.sha256` for each of:
